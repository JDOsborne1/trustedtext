...
func Util_subset_map(_original_map map[string]Block, _keys_to_keep []string) map[string]Block {
	mapped_keys_to_keep := Util_slice_to_bool_map(_keys_to_keep)

	new_map := make(map[string]Block)
...
func Util_anti_set_map[Res Block | bool](_original_map map[string]Res, _keys_to_compare []string) map[string]Res {
